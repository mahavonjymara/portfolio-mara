...
function TechBadge({ name, color, bg, letter }: TechItem) {
  return (
    <div title={name} style={{ display:"flex", flexDirection:"column" as const, alignItems:"center", gap:"0.3rem" }}>
      <div style={{ width:44, height:44, borderRadius:10, background:bg, border:"1.5px solid " + color + "50", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.68rem", fontWeight:800, color:color, fontFamily:"monospace", letterSpacing:"-0.5px", boxShadow:"0 2px 8px " + color + "20" }}>
        {letter}
      </div>
      <span style={{ fontSize:"0.58rem", color:"var(--text3)", fontWeight:500, textAlign:"center" as const }}>{name}</span>
    </div>
  );
}
          ...
          return (
            <div key={skill.name} ref={card.ref}
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"0.6rem", marginBottom:"1rem" }}>
                ...
                <div style={{ fontFamily:"var(--font-syne)", fontWeight:700, fontSize:"1rem", color:"var(--text)" }}>{skill.name}</div>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:"0.6rem", fontSize:"0.78rem", color:"var(--text3)", marginBottom:"1rem" }}>
              ...
              {skill.techs ? (
                <div style={{ display:"flex", flexWrap:"wrap" as const, gap:"0.7rem", marginTop:"0.5rem" }}>
