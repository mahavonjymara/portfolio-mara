      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))", gap:"1.5rem" }}>
        {projects.map((project) => (
          <div key={project.name}
            style={{ background:"rgba(255,255,255,0.6)", border:"1px solid rgba(255,255,255,0.85)", borderRadius:18, padding:"2rem", transition:"all 0.25s", cursor:"pointer", boxShadow:"0 4px 24px rgba(10,22,40,0.08)", backdropFilter:"blur(12px)", position:"relative" as const, overflow:"hidden" }}
            ...
            <div style={{ display:"flex", gap:"0.8rem", marginTop:"1.5rem" }}>
              <a href={project.github} style={{ fontSize:"0.8rem", textDecoration:"none", padding:"0.42rem 1rem", borderRadius:8, border:"1px solid rgba(10,22,40,0.2)", color:"var(--text2)", background:"rgba(255,255,255,0.5)", transition:"all 0.2s" }}>GitHub</a>
              <a href={project.demo} style={{ fontSize:"0.8rem", textDecoration:"none", padding:"0.42rem 1rem", borderRadius:8, border:"1px solid rgba(212,168,83,0.4)", color:"#d4a853", background:"rgba(212,168,83,0.08)", transition:"all 0.2s" }}>Demo</a>
            </div>
          </div>
        ))}
      </div>
